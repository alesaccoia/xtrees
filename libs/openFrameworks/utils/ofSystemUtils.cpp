...
CFURLRef GetOpenFileFromUser(bool bFolder, std::string toFolder)
{
  ...
  if (FSPathMakeRef ((const unsigned char*)(ofToDataPath(toFolder).c_str()), &fsRef, NULL) == noErr)
  {
        if (AECreateDesc (typeFSRef, &fsRef, sizeof(FSRef), &defaultLocation) == noErr)
            defLocPtr = &defaultLocation;
  }
  if (defLocPtr)
        NavCustomControl (dialog, kNavCtlSetLocation, (void*)defLocPtr);
...
ofFileDialogResult ofSystemLoadDialog(string windowTitle, bool bFolderSelection, std::string toFolder){
	...
	//----------------------------------------------------------------------------------------
#ifdef TARGET_OSX
	CFURLRef cfUrl = GetOpenFileFromUser(bFolderSelection, toFolder);
...
ofFileDialogResult ofSystemSaveDialog(string defaultName, string messageName, std::string toFolder){
